...
class ManagePanel(wx.Panel):
    def __init__(self, parent, database):
        ...
        self.Bind(wx.EVT_BUTTON, self.show_questions, self.show_question_button)
...
class CreatePanel(wx.Panel):
    def __init__(self, parent, database):
        ...
        # Question Form
        self.datasets = get_all_datasets(self.database, False)
        wx.StaticText(self, -1, "Choose a dataset to which the question should be assigned", size=(600, -1), pos=(15, 180))
        self.choice_field = wx.Choice(self, -1, choices=self.datasets, size=(600, 200), pos=(15, 200))
        self.choice_field.SetSelection(0)
    ...
    def refresh_datasets(self):
        self.datasets = get_all_datasets(self.database, False)
        self.choice_field.SetItems(self.datasets)
        self.choice_field.SetSelection(0)
...
class LearningPanel(wx.Panel):
    def __init__(self, parent, database):
        ...
        self.choice_field = wx.Choice(self, -1, choices=self.choices, size=(275, -1), pos=(15, 15))
        self.choice_field.SetSelection(0)
        self.start_learning_button = wx.Button(self, label="Start Learning", pos=(300, 15))
        self.Bind(wx.EVT_BUTTON, self.select_dataset, self.start_learning_button)
        self.learning_text_field = wx.StaticText(self, -1, "Question", (150, 200), (400, 200), wx.ALIGN_CENTER)
    ...
    def show_answer(self, event):
        ...
        self.learning_text_field.SetLabel(str(self.questions[self.question_index][3]))
    ...
    def next(self, event):
        ...
        self.learning_text_field.SetLabel(str(self.questions[self.question_index][2]))
        self.right_button.Hide()
    ...
    def select_dataset(self, event):
        ...
        if self.questions is None or len(self.questions) == 0:
            self.learning_text_field.SetLabel('No Questions Available')
        else:
